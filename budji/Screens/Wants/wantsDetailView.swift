...
struct wantsDetailView: View {
    ...
    private func populateCategories() {
        ...
        viewModel.options = Array(Set(viewModel.amountTwo.map { $0.category }))
    }
        ...
        VStack(spacing: 30){
                    ...
                    Menu {
                        ForEach(months, id: \.self) { month in
                            Button(action: {
                                ...
                                viewModel.getWantsData(currentMonth: month)
            ...
            if viewModel.isDataEmpty {
                ...
                VStack(alignment: .leading) {
                    ...
                    //                }
                    List(viewModel.amountTwo.filter { exp in
                ...
                .onAppear(perform: {
                    currentMonth()
                    viewModel.getWantsData(currentMonth: monthText)
                    populateCategories()
                })
            }
        }
...
struct WantsDetailView_Previews: PreviewProvider {
    @State static var sampleExpenses = expense.sampleExpenses
    static var previews: some View {
        wantsDetailView(amount: $sampleExpenses,amountTwo: $sampleExpenses, amountThree: $sampleExpenses)
    }
}
