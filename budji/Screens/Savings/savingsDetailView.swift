...
struct savingsDetailView: View {
    ...
    private func populateCategories() {
        ...
        viewModel.options = Array(Set(viewModel.amountThree.map { $0.category }))
    }
        ...
        VStack(spacing: 30){
                    ...
                    Menu {
                        ForEach(months, id: \.self) { month in
                            Button(action: {
                                ...
                                viewModel.getSavingsData(currentMonth: month)
            ...
            if viewModel.isDataEmpty {
                ...
                VStack(alignment: .leading) {
                    ...
                    //                }
                    List(viewModel.amountThree.filter { exp in
                ...
                .onAppear(perform: {
                    currentMonth()
                    viewModel.getSavingsData(currentMonth: monthText)
                    populateCategories()
                })
            }
        }
...
struct SavingDetailView_Previews: PreviewProvider {
    ...
    static var previews: some View {
        savingsDetailView(amount: $sampleExpenses,amountTwo: $sampleExpenses, amountThree: $sampleExpenses)
    }
}
